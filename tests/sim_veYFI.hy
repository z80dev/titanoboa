...
(let [timestamp boa.env.vm.patch.timestamp
  ...
  (with [(boa.env.prank DOGGIE)] (veYFI.modify_lock (** 10 18) 2y_lock DOGGIE)))

(timeit "lock veYFI")

(defn warp_week [[n 1]]
  (setv boa.env.vm.patch.timestamp (+ (* n WEEK) boa.env.vm.patch.timestamp))
  (setv boa.env.vm.patch.block_number (+ (* n 40_000) boa.env.vm.patch.block_number)))

(for [i (range START_TIME
               (int (+ START_TIME (* 1.2 YEAR)))
               WEEK)]
  (warp_week)
  (veYFI.checkpoint)
